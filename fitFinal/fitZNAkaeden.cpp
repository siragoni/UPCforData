...
void fitZNA(const char* AnalysisName){
   ...
   */
  TH1F *fZNCEnergyAgainstEntriesH = (TH1F*)listings->FindObject("fZNAEnergyAgainstEntriesH");
  fZNCEnergyAgainstEntriesH->Rebin(150);
  ...
  /* - This distribution is modelled as the sum of many different gaussians.
     ...
     -
   */
  RooRealVar      x             ("x"  ,           "x",   -200.*2500./190.,  1400.*2500./190.);
  RooRealVar      mean0Neutrons ("mean0Neutrons",  "",     0*2500./190.,   -5*2500./190.,    5*2500./190. );
  RooRealVar      sigma0Neutrons("sigma0Neutrons", "",    20*2500./190.,    0*2500./190.,   25*2500./190. );
  RooRealVar      mean1Neutron  ("mean1Neutron",   "",   200*2500./190.,   190*2500./190., 220*2500./190. );
  RooRealVar      sigma1Neutron ("sigma1Neutron",  "",    30*2500./190.,    20*2500./190.,  50*2500./190. );
  // RooFormulaVar   sigma1Neutron ("sigma1Neutron",  "",    "sqrt(2)*sigma0Neutrons", sigma0Neutrons);
  // RooFormulaVar   sigma2Neutrons("sigma2Neutrons", "",    "sqrt(2)*sigma1Neutron", sigma1Neutron);
  // RooFormulaVar   sigma3Neutrons("sigma3Neutrons", "",    "sqrt(3)*sigma1Neutron", sigma1Neutron);
  RooFormulaVar   mean2Neutrons ("mean2Neutrons",  "",    "2*mean1Neutron" ,  mean1Neutron);
  RooFormulaVar   mean3Neutrons ("mean3Neutrons",  "",    "3*mean1Neutron" ,  mean1Neutron);
  RooFormulaVar   sigma2Neutrons("sigma2Neutrons", "",    "sqrt(2*(sigma1Neutron*sigma1Neutron-sigma0Neutrons*sigma0Neutrons)+sigma0Neutrons*sigma0Neutrons)", RooArgList(sigma1Neutron, sigma0Neutrons) );
  RooFormulaVar   sigma3Neutrons("sigma3Neutrons", "",    "sqrt(3*(sigma1Neutron*sigma1Neutron-sigma0Neutrons*sigma0Neutrons)+sigma0Neutrons*sigma0Neutrons)", RooArgList(sigma1Neutron, sigma0Neutrons) );
  ...
  /* - The respective values for the global pdf's normalizations
     -
   */
  RooRealVar n0Neutrons    ("n0Neutrons",     "",   1000,    500., 20000.);
  RooRealVar n1Neutron     ("n1Neutron",      "",   2000,    500.,  2500.);
  RooRealVar n2Neutrons    ("n2Neutrons",     "",    100,    100.,  2000.);
  RooRealVar n3Neutrons    ("n3Neutrons",     "",     50,     30.,  1000.);
