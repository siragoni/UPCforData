...
void fitZNC(const char* AnalysisName){
  ...
  /* - This distribution is modelled as the sum of many different gaussians.
     ...
     -
     - *2500./190. multiplicative correction factor for LHC18q+r.
   */
  ...
  RooRealVar      x             ("x"  ,           "x",   -200.*2500./190.,  1000.0*2500./190.);
  RooRealVar      mean0Neutrons ("mean0Neutrons",  "",      0.,    -50.,   50. );
  RooRealVar      sigma0Neutrons("sigma0Neutrons", "",    450.,    400.,  500. );
  RooRealVar      mean1Neutron  ("mean1Neutron",   "",   2500.,   2200., 2800. );
  RooRealVar      sigma1Neutron ("sigma1Neutron",  "",    400.,    200.,  800. );
  RooRealVar      mean2Neutrons ("mean2Neutrons",  "",   5000.,   4800., 5200. );
  RooRealVar      sigma2Neutrons("sigma2Neutrons", "",    400.,    200.,  800. );
  RooRealVar      mean3Neutrons ("mean3Neutrons",  "",   7500.,   7400., 7900. );
  RooRealVar      sigma3Neutrons("sigma3Neutrons", "",    500.,    300.,  800. );
  ...
  /* - The respective values for the global pdf's normalizations
     -
   */
  RooRealVar n0Neutrons    ("n0Neutrons",     "",    700,    700.,  1000.);
  RooRealVar n1Neutron     ("n1Neutron",      "",    2000,    1000.,  3000.);
  RooRealVar n2Neutrons    ("n2Neutrons",     "",    1000,    800.,  2000.);
  RooRealVar n3Neutrons    ("n3Neutrons",     "",    900,     700.,  1000.);
  ...
  RooFitResult* fFitResult = fit->fitTo( *fZNCEnergyAgainstEntriesRoofitH,
                                          RooFit::Save(),
                                          Range(-190*2500./190., 700*2500./190.)
                                         );
