...
void drawMeasurement(int i, double m[5], char label[2][100], int aux[6],
  ...
  if ( i < 3) {
    ...
    lowY = (i+1)*vstep+vstep*0.3;
    uppY = (i+2)*vstep+vstep*0.3;
  }
...
void bs() {
  ...
  Double_t SysThetaHeALICE    = 0.069;
  Double_t SysPhiHeALICE      = 0.014;
  ...
  TCanvas* canvas = new TCanvas("canvas", "canvas", 200, 0,
				TMath::Nint(width), TMath::Nint(height*1.2));
  ...
  canvas->SetTopMargin(12.0/height);
  canvas->Draw();
  canvas->cd();
  ...
  latex->DrawLatex(0.2,0.92,"ALICE, Pb-Pb #sqrt{s_{NN}} = 5.02 TeV");
  latex->SetTextSize(0.045);
  // latex->DrawLatex(0.05,0.1,"(a) = r_{00}^{04}, (b) = r_{1, -1}^{04}");
  latex->DrawLatex(0.37,0.1," r_{00}^{04},  r_{1, -1}^{04}");
