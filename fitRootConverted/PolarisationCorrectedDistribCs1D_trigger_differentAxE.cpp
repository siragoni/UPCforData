...
void PolarisationCorrectedDistribCs1D( Int_t RangeSelectionMode = 0 ){
   ...
   */
  TH1F* fReconCosThetaH = (TH1F*)listings->FindObject( Form("fCosThetaCsFrameTwentyfiveBinsTriggerH_%d", (RangeSelectionMode-1)) );
  ...
  TH1F* fReconPhiH = (TH1F*)listings->FindObject( Form("fPhiCsFrameTwentyfiveBinsTriggerH_%d", (RangeSelectionMode-1)) );
  TH1F* fGenerPhiH = (TH1F*)listings->FindObject("fMCPhiCsFrameTwentyfiveBinsH");
  fReconPhiH->Sumw2();
  fGenerPhiH->Sumw2();

  TH1F* fReconTildePhiH = (TH1F*)listings->FindObject( Form("fTildePhiCsFrameTwentyfiveBinsTriggerH_%d", (RangeSelectionMode-1) ) );
  ...
  //   fileDataRawTildePhi = new TFile( Form("pngResults/%d-%2.2d-%2.2d/TildePhiCS/TildePhiCsFrame_5.root", d.GetYear(), d.GetMonth(), d.GetDay() ) );
    ...
    fileDataRawCosTheta = new TFile( Form("pngResults/PolTrigger%d/CosThetaCS/CosThetaCSFrame.root",   RangeSelectionMode ) );
    fileDataRawPhi      = new TFile( Form("pngResults/PolTrigger%d/PhiCS/PhiCsFrame.root",             RangeSelectionMode ) );
    fileDataRawTildePhi = new TFile( Form("pngResults/PolTrigger%d/TildePhiCS/TildePhiCsFrame.root",   RangeSelectionMode ) );
  }
  ...
  //   f.Close();
    ...
    TFile f( Form("pngResults/PolTrigger%d/1Dresults/PolarisationCorrectedCs1D_trigger_differentAxE.root", RangeSelectionMode), "recreate");
    acceptanceCosTheta->Write();
    CorrCosThetaH     ->Write();
    acceptancePhi     ->Write();
    CorrPhiH          ->Write();
