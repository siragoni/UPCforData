...
void PolarisationCorrectedDistribHe1D( Int_t RangeSelectionMode = 0 ){
  ...
  TH1F* fReconCosThetaH = (TH1F*)SavedFile->Get("fCosThetaReconstructed");
  TH1F* fGenerCosThetaH = (TH1F*)listings2->FindObject("fMCCosThetaCsFrameTwentyfiveBinsH");
  ...
  if        ( RangeSelectionMode == 0 ) {
    ...
    TFile f("pngResults/PolarisationCorrectedHe1D_AxE.root", "recreate");
    acceptanceCosTheta->Write();
    CorrCosThetaH     ->Write();
    acceptancePhi     ->Write();
    CorrPhiH          ->Write();
