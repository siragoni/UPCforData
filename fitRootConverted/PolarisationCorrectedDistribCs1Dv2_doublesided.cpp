...
void PolarisationCorrectedDistribCs1D( Int_t RangeSelectionMode = 0 ){
  ...
  if (        RangeSelectionMode == 0 ) {
    ...
    fileDataRawCosTheta = new TFile( "pngResults/CosThetaCsFrame_DSCB.root" );
  ...
  if        ( RangeSelectionMode == 0 ) {
    TDatime d;
    TFile f(Form("pngResults/%d-%2.2d-%2.2d/1DresultsV2/PolarisationCorrectedCs1Dv2_doublesided.root",   d.GetYear(), d.GetMonth(), d.GetDay()), "recreate");
    // TFile f("pngResults/PolarisationCorrectedCs1D.root", "recreate");
