...
void PolarisationCorrectedDistribHe1D( Int_t RangeSelectionMode = 0 ){
  ...
  fReconCosThetaH->Rebin(25);
  fGenerCosThetaH->Rebin(25);
  ...
  if (        RangeSelectionMode == 0 ) {
    fileDataRawCosTheta = new TFile( Form("pngResults/%d-%2.2d-%2.2d/CosThetaHE/CosThetaHeFrame_purity70.root",   d.GetYear(), d.GetMonth(), d.GetDay() ) );
  ...
  if        ( RangeSelectionMode == 0 ) {
    TFile f("pngResults/PolarisationCorrectedHe1D_purity70.root", "recreate");
