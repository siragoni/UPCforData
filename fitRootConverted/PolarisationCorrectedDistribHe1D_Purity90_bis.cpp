...
void PolarisationCorrectedDistribHe1D( Int_t RangeSelectionMode = 0 ){
   ...
   */
  TH1F* fReconCosThetaH = (TH1F*)listings->FindObject("fCosThetaHelicityFrameJPsiH");
  TH1F* fGenerCosThetaH = (TH1F*)listings->FindObject("fMCCosThetaHelicityFrameJPsiH");
  fReconCosThetaH->Rebin(50);
  fGenerCosThetaH->Rebin(50);
  ...
  if (        RangeSelectionMode == 0 ) {
    fileDataRawCosTheta = new TFile( Form("pngResults/%d-%2.2d-%2.2d/CosThetaHE/CosThetaHeFrame_purity90_bis.root",   d.GetYear(), d.GetMonth(), d.GetDay() ) );
  ...
  if        ( RangeSelectionMode == 0 ) {
    TFile f("pngResults/PolarisationCorrectedHe1D_purity90_bis.root", "recreate");
