...
void PolarisationCorrectedDistribHe1D( Int_t RangeSelectionMode = 0 ){
  ...
  TFile* fileList = new TFile("AnalysisResultsMC_poltrigger.root");
  TDirectory* dir = fileList->GetDirectory("MyTask");
  TList* listings;
  dir->GetObject("MyOutputContainer", listings);

  TFile* fileList2 = new TFile("MCtrainResults/2019-09-17/kCohJpsiToMu/AnalysisResults.root");
   ...
   */
  TH1F* fReconCosThetaH = (TH1F*)listings->FindObject( Form("fCosThetaHelicityFrameTwentyfiveBinsTriggerH_%d", (RangeSelectionMode-1)) );
  ...
  TH1F* fReconPhiH = (TH1F*)listings->FindObject( Form("fPhiHelicityFrameTwentyfiveBinsTriggerH_%d", (RangeSelectionMode-1)) );
  TH1F* fGenerPhiH = (TH1F*)listings2->FindObject("fMCPhiHelicityFrameTwentyfiveBinsH");
  fReconPhiH->Sumw2();
  fGenerPhiH->Sumw2();

  TH1F* fReconTildePhiH = (TH1F*)listings->FindObject( Form("fTildePhiHelicityFrameTwentyfiveBinsTriggerH_%d", (RangeSelectionMode-1) ) );
  ...
  if (        RangeSelectionMode == 0 ) {
    ...
    fileDataRawTildePhi = new TFile( Form("pngResults/%d-%2.2d-%2.2d/TildePhiHE/TildePhiCeFrame.root",   d.GetYear(), d.GetMonth(), d.GetDay() ) );
  ...
  //   fileDataRawTildePhi = new TFile( Form("pngResults/%d-%2.2d-%2.2d/TildePhiCS/TildePhiCsFrame_5.root", d.GetYear(), d.GetMonth(), d.GetDay() ) );
    ...
    fileDataRawCosTheta = new TFile( Form("pngResults/PolTrigger%d/CosThetaHE/CosThetaHeFrame.root",   RangeSelectionMode ) );
    fileDataRawPhi      = new TFile( Form("pngResults/PolTrigger%d/PhiHE/PhiHeFrame.root",             RangeSelectionMode ) );
    fileDataRawTildePhi = new TFile( Form("pngResults/PolTrigger%d/TildePhiHE/TildePhiHeFrame.root",   RangeSelectionMode ) );
  }
  ...
  //   f.Close();
    ...
    TFile f( Form("pngResults/PolTrigger%d/1Dresults/PolarisationCorrectedHe1D_trigger_differentAxE.root", RangeSelectionMode), "recreate");
    acceptanceCosTheta->Write();
    CorrCosThetaH     ->Write();
    acceptancePhi     ->Write();
    CorrPhiH          ->Write();
