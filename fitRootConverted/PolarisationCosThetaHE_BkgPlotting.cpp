 ...
 * -
 */
void fBkgPolFit(TH1F* histoToBeFit, Int_t selectionFlag_)//, Double_t &bookKeeping[5])
{
  ...
  histoToBeFit->SetLineStyle(kSolid);
  histoToBeFit->SetLineWidth(3);
  histoToBeFit->SetMarkerStyle(kFullCircle);
  histoToBeFit->SetMarkerColor(kBlue);
  histoToBeFit->SetMarkerSize(1);
  histoToBeFit->GetXaxis()->SetTitle("M_{#mu#mu} [GeV/#it{c}^{2}]");
  gPad->SetMargin(0.13,0.01,0.12,0.01);
  // gPad->SetLogy();
  gStyle->SetOptFit(0);
  gStyle->SetOptStat(0);
  gPad->SetTitle(  Form(  ";M_{#mu#mu} (GeV/c^{2});Counts / (%.0f MeV/c^{2})",
                           histoToBeFit->GetXaxis()->GetBinWidth(1)*1000.  )  );
  /* - Beautifying is starting now.
     -
   */
  histoToBeFit->GetXaxis()->SetTitleOffset(1.25);
  // fInvariantMassDistributionH->GetYaxis()->SetTitleOffset(1.25);
  histoToBeFit->GetYaxis()->SetTitleOffset(1.45);
  histoToBeFit->GetXaxis()->SetTitleSize(0.045);
  histoToBeFit->GetYaxis()->SetTitleSize(0.045);
  histoToBeFit->GetXaxis()->SetLabelSize(0.045);
  histoToBeFit->GetYaxis()->SetLabelSize(0.045);
  histoToBeFit->GetXaxis()->SetTitleFont(42);
  histoToBeFit->GetYaxis()->SetTitleFont(42);
  histoToBeFit->GetXaxis()->SetLabelFont(42);
  histoToBeFit->GetYaxis()->SetLabelFont(42);
  histoToBeFit->GetXaxis()->SetNdivisions(408);
  histoToBeFit->GetYaxis()->SetRangeUser(0.0000000000001, histoToBeFit->GetMaximum()*1.3);
  histoToBeFit->GetXaxis()->SetRangeUser(2, 6);
  histoToBeFit->Draw("PEsame");
  TLatex* latexBKG = new TLatex();
  latexBKG->SetTextSize(0.05);
  latexBKG->SetTextFont(42);
  latexBKG->SetTextAlign(11);
  latexBKG->SetNDC();
  latexBKG->DrawLatex(0.17,0.94,"ALICE Simulation, PbPb #sqrt{s_{NN}} = 5.02 TeV");
  latexBKG->SetTextSize(0.045);
  // latexBKG->DrawLatex(0.55,0.84,"UPC, #it{L} = 235 ub^{-1}");
  latexBKG->DrawLatex(0.55,0.84,"STARlight");
  latexBKG->DrawLatex(0.55,0.78,Form("#color[2]{%.3f < cos(#theta) < %.3f}", -1 + (Double_t)selectionFlag_ * 0.08, -1 + ((Double_t)selectionFlag_ + 1.00) * 0.08));
  gPad    ->SaveAs(Form("pngResults/BKG_%d.png", selectionFlag_), "recreate");
...
void fitJPsiTemplateMC(const int selectionFlag = 0){
  ...
  // fTwoGammaToMuHigh   = (TH1F*)listingsMC[7]->FindObject(Form("fInvariantMassDistributionOnlyCosThetaHeFrameTwentyfiveBinsH_%d", selectionFlag));
  /* - Rebin
     -
   */
  ...
  fBkgPolFit             (fTwoGammaToMuHigh, selectionFlag);//, CBparameters[4]);
  // }
}
//_____________________________________________________________________________
/* - Fit function for the ZNC plots.
...
void fitJPsiTemplate(const int selectionFlag, Int_t SignalRangeModeFlag = 0){
   ...
   * - Derivative
   */
  Double_t LeftDerivative  = 0;
  Double_t RightDerivative = 0;
  LeftDerivative  = JPsiPeakFit->Derivative( fFitInvMass->GetParameter(1) + fFitInvMass->GetParameter(2) * 1.04 );
  RightDerivative = JPsiPeakFit->Derivative( fFitInvMass->GetParameter(1) + fFitInvMass->GetParameter(2) * 1.12 );
  cout << "Left  Derivative = " << LeftDerivative  << endl;
  cout << "Right Derivative = " << RightDerivative << endl;
  ...
  latex->DrawLatex(0.55,0.72,Form("Left  Derivative = %.3f",LeftDerivative));
  latex->DrawLatex(0.55,0.66,Form("Right Derivative = %.3f",RightDerivative));
  ...
  latex->DrawLatex(0.55,0.60,Form("N_{J/#psi} = %.0f #pm %.0f",        numberOfTotalJPsi,  numberOfTotalJPsiErr ));//fFitInvMass->GetParameter(0) *fFitInvMass->GetParError(15)/0.05 ) );
  latex->DrawLatex(0.55,0.54,Form("N_{#psi(2S)} = %.0f #pm %.0f",      numberOfTotalPsi2s, numberOfTotalPsi2sErr));//fFitInvMass->GetParameter(5) *fFitInvMass->GetParError(16)/0.05 ) );
  latex->DrawLatex(0.55,0.48,Form("N_{#gamma#gamma} = %.0f #pm %.0f",  numberOfTotalBkg,   numberOfTotalBkgErr  ));//fFitInvMass->GetParameter(10)*fFitInvMass->GetParError(17)/0.05 ) );
