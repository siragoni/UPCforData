...
void CreateCosThetaTh1(const char* AnalysisName, Int_t SignalRangeMode = 0){
  ...
  for (size_t iCosThetaBins = 8; iCosThetaBins < 32; iCosThetaBins++) {
    // for (size_t iPhiBins = 0; iPhiBins < 10; iPhiBins++) {
      ...
      CosThetaAfterSignalExtractionH->Fill(  -1 + 0.025 + (Double_t)iCosThetaBins * 0.05,
                                            // -3.14 + 0.314 * ( 2.0 * (Double_t)iPhiBins + 1.0) ,
                                            JPsiPeakValue
                                            );

      CosThetaAfterSignalExtractionErrorsH->Fill(  -1 + 0.025 + (Double_t)iCosThetaBins * 0.05,
                                                   JPsiPeakValue
                                                   );
      CosThetaGammaGammaErrorsH->Fill(  -1 + 0.025 + (Double_t)iCosThetaBins * 0.05,
                                        BkgValue
                                        );
  ...
  if      ( SignalRangeMode == 0 ) {
    TFile f("pngResults/CosThetaHeFrame_purity70.root",   "recreate");
