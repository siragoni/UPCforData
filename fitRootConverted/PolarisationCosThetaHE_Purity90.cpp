...
void fitJPsiTemplateMC(const int selectionFlag = 0){
  ...
  // }
  fCohJpsiToMu        = (TH1F*)listingsMC[0]->FindObject(Form("fInvariantMassDistributionOnlyCosThetaHeFrameTwentyfiveBinsH_%d", 4+2*selectionFlag));
  fCohJpsiToMu2       = (TH1F*)listingsMC[0]->FindObject(Form("fInvariantMassDistributionOnlyCosThetaHeFrameTwentyfiveBinsH_%d", 4+2*selectionFlag+1));
...
void fitJPsiTemplate(const int selectionFlag, Int_t SignalRangeModeFlag = 0){
  ...
  fInvariantMassDistributionH  = (TH1F*)listings->FindObject( Form("fInvariantMassDistributionOnlyCosThetaHeFrameTwentyfiveBinsH_%d", 4+2*selectionFlag) );
  fInvariantMassDistributionH2 = 0x0;
  fInvariantMassDistributionH2 = (TH1F*)listings->FindObject( Form("fInvariantMassDistributionOnlyCosThetaHeFrameTwentyfiveBinsH_%d", 4+2*selectionFlag+1) );
  ...
  latex->DrawLatex(0.55,0.78,Form("#color[2]{%.3f < cos(#theta) < %.3f}", -0.6 + (Double_t)selectionFlag * 0.16, -1 + ((Double_t)selectionFlag + 1.00) * 0.16));
...
void CreateCosThetaTh1(const char* AnalysisName, Int_t SignalRangeMode = 0){
  ...
  for (size_t iCosThetaBins = 0; iCosThetaBins < 8; iCosThetaBins++) {
    // for (size_t iPhiBins = 0; iPhiBins < 10; iPhiBins++) {
      ...
      CosThetaAfterSignalExtractionH->Fill(  -0.52 + (Double_t)iCosThetaBins * 0.16,
                                            // -3.14 + 0.314 * ( 2.0 * (Double_t)iPhiBins + 1.0) ,
                                            JPsiPeakValue
                                            );

      CosThetaAfterSignalExtractionErrorsH->Fill(  -0.52 + (Double_t)iCosThetaBins * 0.16,
                                                   JPsiPeakValue
                                                   );
      CosThetaGammaGammaErrorsH->Fill(  -0.52 + (Double_t)iCosThetaBins * 0.16,
                                        BkgValue
                                        );
  ...
  if      ( SignalRangeMode == 0 ) {
    TFile f("pngResults/CosThetaHeFrame_purity90.root",   "recreate");
