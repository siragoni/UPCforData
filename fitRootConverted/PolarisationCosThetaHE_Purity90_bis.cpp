...
TH1F *fInvariantMassDistributionH2 = 0x0;
...
void fitJPsiTemplateMC(const int selectionFlag = 0){
  ...
  // }
  // fCohJpsiToMu        = (TH1F*)listingsMC[0]->FindObject(Form("fInvariantMassDistributionOnlyCosThetaForSignalExtractionHelicityFrameH_%d", selectionFlag));
  fCohJpsiToMu        = (TH1F*)listingsMC[0]->FindObject(Form("fInvariantMassDistributionOnlyCosThetaForSignalExtractionHelicityFrameH_%d", 2*selectionFlag));
  fCohJpsiToMu2       = (TH1F*)listingsMC[0]->FindObject(Form("fInvariantMassDistributionOnlyCosThetaForSignalExtractionHelicityFrameH_%d", 2*selectionFlag+1));
  fCohJpsiToMu->Add(fCohJpsiToMu2);
...
void fitJPsiTemplate(const int selectionFlag, Int_t SignalRangeModeFlag = 0){
  ...
  fInvariantMassDistributionH  = 0x0;
  fInvariantMassDistributionH  = (TH1F*)listings->FindObject( Form("fInvariantMassDistributionOnlyCosThetaForSignalExtractionHelicityFrameH_%d", 2*selectionFlag) );
  fInvariantMassDistributionH2 = 0x0;
  fInvariantMassDistributionH2 = (TH1F*)listings->FindObject( Form("fInvariantMassDistributionOnlyCosThetaForSignalExtractionHelicityFrameH_%d", 2*selectionFlag+1) );
  fInvariantMassDistributionH->Add(fInvariantMassDistributionH2);
  ...
  latex->DrawLatex(0.55,0.78,Form("#color[2]{%.3f < cos(#theta) < %.3f}", -1 + (Double_t)selectionFlag * 0.05, -1 + ((Double_t)selectionFlag + 1.00) * 0.05));
...
void CreateCosThetaTh1(const char* AnalysisName, Int_t SignalRangeMode = 0){
  ...
  for (size_t iCosThetaBins = 4; iCosThetaBins < 16; iCosThetaBins++) {
    // for (size_t iPhiBins = 0; iPhiBins < 10; iPhiBins++) {
      ...
      CosThetaAfterSignalExtractionH->Fill(  -1 + 0.050 + (Double_t)iCosThetaBins * 0.1,
                                            // -3.14 + 0.314 * ( 2.0 * (Double_t)iPhiBins + 1.0) ,
                                            JPsiPeakValue
                                            );

      CosThetaAfterSignalExtractionErrorsH->Fill(  -1 + 0.050 + (Double_t)iCosThetaBins * 0.1,
                                                   JPsiPeakValue
                                                   );
      CosThetaGammaGammaErrorsH->Fill(  -1 + 0.050 + (Double_t)iCosThetaBins * 0.1,
                                        BkgValue
                                        );
  ...
  if      ( SignalRangeMode == 0 ) {
    TFile f("pngResults/CosThetaHeFrame_purity90_bis.root",   "recreate");
