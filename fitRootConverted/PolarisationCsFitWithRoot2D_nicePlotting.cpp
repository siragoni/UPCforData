...
/* - Fit function for the helicity case. It is basically a parabolic fit...
   -
 */
void PolarisationCsFitWithRoot2D(){

  TDatime d;
  TFile* data = new TFile(Form("pngResults/%d-%2.2d-%2.2d/2DCS/PolarisationCorrectedCs2D.root", d.GetYear(), d.GetMonth(), d.GetDay() ) );
  // TFile* data = new TFile("pngResults/2019-09-18/2DCS/PolarisationCorrectedCs2D.root");
  TH2F *fCorrectedShape = (TH2F*) data->Get("RawH");
  // TF2* ParabolicFit = new TF2("ParabolicFit",helicity2D,-0.3, 0.6, -3.1, 3.1, 4);
  ...
  fCorrectedShape->GetXaxis()->SetTitleOffset(1.15);
  // fCorrectedShape->GetYaxis()->SetTitleOffset(1.25);
  fCorrectedShape->GetYaxis()->SetTitleOffset(1.);
  fCorrectedShape->GetXaxis()->SetTitleSize(0.045);
  fCorrectedShape->GetYaxis()->SetTitleSize(0.045);
  fCorrectedShape->GetXaxis()->SetLabelSize(0.045);
  fCorrectedShape->GetYaxis()->SetLabelSize(0.045);
  ...
  fCorrectedShape->SetTitle( ";cos(#theta); #phi" );
  fCorrectedShape->Draw("colZ");
  // ParabolicFit   ->Draw("cont1 SAME");
  ParabolicFit   ->Draw("SAME");
  ...
  latex->DrawLatex(0.55,0.84,"UPC, Run 2, CS");
  ...
  gPad->SaveAs("pngResults/PolFitWithRootCs2D.png",         "RECREATE");
