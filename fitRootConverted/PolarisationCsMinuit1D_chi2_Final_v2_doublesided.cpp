...
void PolarisationHeMinuit1D( Int_t SignalRangeSelectionMode = 0, Int_t FitRangeMode = 0 ){
  ...
  if        ( SignalRangeSelectionMode == 0 ) {
    file1D = new TFile(Form("pngResults/%d-%2.2d-%2.2d/1DresultsV2/PolarisationCorrectedCs1Dv2_doublesided.root",   d.GetYear(), d.GetMonth(), d.GetDay() ) );
  ...
  Model->SetNpx(500);
  Model->Draw("same");
  if ( SignalRangeSelectionMode == 0 || FitRangeMode == 0 ) gPad->SaveAs("pngResults/CosThetaCsMinuit_doublesided.png", "recreate");
  gPad->SaveAs(Form("pngResults/CosThetaHeMinuit_SigEx_%d_FitRange_%d_CS_DSCB.png", SignalRangeSelectionMode, FitRangeMode), "recreate");
  ...
  Model2->SetNpx(500);
  Model2->Draw("same");
  if ( SignalRangeSelectionMode == 0 || FitRangeMode == 0 ) gPad->SaveAs("pngResults/PhiCsMinuit_DSCB.png", "recreate");
  gPad->SaveAs(Form("pngResults/PhiHeMinuit_SigEx_%d_FitRange_%d_CS_DSCB.png", SignalRangeSelectionMode, FitRangeMode), "recreate");
  ...
  Model3->SetNpx(500);
  Model3->Draw("same");
  if ( SignalRangeSelectionMode == 0 || FitRangeMode == 0 ) gPad->SaveAs("pngResults/TildePhiCsMinuit_DSCB.png", "recreate");
  gPad->SaveAs(Form("pngResults/TildePhiHeMinuit_SigEx_%d_FitRange_%d_CS_DSCB.png", SignalRangeSelectionMode, FitRangeMode), "recreate");
  ...
  // // gMinuit->GetParameter(5, LambdaThetaPhi,   LambdaThetaPhiErr  );
  // new TCanvas;
  // TFile ContourFile( "pngResults/ContourFileCsV2.root", "recreate" );
