...
Double_t CosTheta(Double_t *x, Double_t *par) {
  ...
  // returnValue              = par[4] * 3. * 0.987632 * returnValue / ( 3. + par[0] );
...
void PolarisationHeMinuit1D( Int_t SignalRangeSelectionMode = 0, Int_t FitRangeMode = 0 ){
  ...
  if        ( SignalRangeSelectionMode == 0 ) {
    file1D = new TFile(Form("pngResults/%d-%2.2d-%2.2d/1DresultsV2/PolarisationCorrectedCs1Dv2_AxE.root",   d.GetYear(), d.GetMonth(), d.GetDay() ) );
