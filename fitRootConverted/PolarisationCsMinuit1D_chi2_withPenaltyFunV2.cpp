...
void PolarisationHeMinuit1D( Int_t SignalRangeSelectionMode = 0, Int_t FitRangeMode = 0 ){
  ...
  if        ( SignalRangeSelectionMode == 0 ) {
    ...
    file1D = new TFile(Form("pngResults/%d-%2.2d-%2.2d/1DresultsV2/PolarisationCorrectedCs1Dv2.root",   d.GetYear(), d.GetMonth(), d.GetDay() ) );
  ...
  Double_t PhiLowLimit        = -3.15;
  Double_t PhiUpperLimit      = +3.15;
  ...
  gMinuit->DefineParameter(3, "LambdaPhi",           0, 0.01,    -1., 1.   );
  ...
  gMinuit->DefineParameter(5, "LambdaThetaPhi",      0, 0.01,    -1., 1.   );
  ...
  gPad->SaveAs(Form("pngResults/PhiCsMinuit_SigEx_%d_FitRange_%d_CS.png", SignalRangeSelectionMode, FitRangeMode), "recreate");
