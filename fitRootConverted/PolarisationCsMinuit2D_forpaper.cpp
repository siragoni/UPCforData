...
void PolarisationCsMinuit2D( Int_t SignalRangeSelectionMode = 0, Int_t FitRangeMode = 0 ){
  ...
  Distr2D->SetTitle( ";cos(#theta); #varphi" );
  Distr2D->Draw("colZ");
  ...
  new TCanvas;
  // TCanvas *c_super = new TCanvas("c_super","comparing modes",100,100,900,600);
  gPad->SetMargin(0.13,0.13,0.12,0.12);
  // Distr2D->GetXaxis()->SetTitleOffset(1.15);
  // // Distr2D->GetYaxis()->SetTitleOffset(1.25);
  // Distr2D->GetYaxis()->SetTitleOffset(1.);
  ...
  latex2->SetNDC();
  // latex2->DrawLatex(0.17,0.94,"ALICE Public, PbPb #sqrt{s_{NN}} = 5.02 TeV");
  latex2->DrawLatex(0.12,0.94,"ALICE Public, PbPb #sqrt{s_{NN}} = 5.02 TeV");
  latex2->SetTextSize(0.055);
  latex2->DrawLatex(0.7,0.94,"Collins-Soper");
  // latex2->DrawLatex(0.55,0.84,"UPC, Run 2 dataset, HE");

  gPad->SaveAs("pngResults/Inverted2DmapsCS.pdf",  "RECREATE");
