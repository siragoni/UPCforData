...
void PolarisationHeMinuit1D( Int_t SignalRangeSelectionMode = 0, Int_t FitRangeMode = 0 ){
  ...
  if        ( SignalRangeSelectionMode == 0 ) {
    file1D = new TFile(Form("pngResults/%d-%2.2d-%2.2d/1Dresults/PolarisationCorrectedHe1D_GammaGamma.root",   d.GetYear(), d.GetMonth(), d.GetDay() ) );
  ...
  gMinuit->DefineParameter(1, "NormalTheta", 800, 10,  600, 1000);
  // gMinuit->DefineParameter(2, "NormalisPhi", 4137, 100,  4000, 4400);
  gMinuit->DefineParameter(2, "NormalisPhi",      1000,  10,  600., 2500);
  gMinuit->DefineParameter(3, "LambdaPhi",           0, 0.1,    -2, 2   );
  gMinuit->DefineParameter(4, "NormalisTildePhi", 1000,  10,  600., 2500);
