...
Double_t CosTheta(Double_t *x, Double_t *par) {
  ...
  // returnValue              = par[4] * 3. * 0.988605 * returnValue / ( 3. + par[0] );
  return   returnValue;
}
//______________________________________________
/* -
...
void PolarisationHeMinuit1D( Int_t SignalRangeSelectionMode = 0, Int_t FitRangeMode = 0 ){
  ...
  if        ( SignalRangeSelectionMode == 0 ) {
    file1D = new TFile(Form("pngResults/%d-%2.2d-%2.2d/1DresultsV2/PolarisationCorrectedHe1Dv2_AxE.root",   d.GetYear(), d.GetMonth(), d.GetDay() ) );
  ...
  gMinuit->DefineParameter(3, "LambdaPhi",           0, 0.01,    -2, 2   );
  gMinuit->DefineParameter(4, "NormalisTildePhi", 8300, 100,  7000, 9500);
  // gMinuit->DefineParameter(4, "NormalisTildePhi", 4500, 100,  3500, 6000);
