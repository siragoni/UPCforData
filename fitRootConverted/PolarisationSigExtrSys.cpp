...
void PolarisationSigExtrSystematics(){
  ...
  for ( Int_t SigExBin = 0; SigExBin < 6; SigExBin++ ) {
    for ( Int_t FitRangeMode = 0; FitRangeMode < 3; FitRangeMode++ ) {
      FitResultFile[SigExBin][FitRangeMode] = new TFile(
            Form( "pngResults/%d-%2.2d-%2.2d/1Dresults/Parameters_SigEx_%d_FitRange_%d_HE.root",
  ...
  Double_t MaxLambdaThetaPlusError     = *max_element( LambdaThetaPlusError,    LambdaThetaPlusError    + 18 );
  Double_t MaxLambdaPhiPlusError       = *max_element( LambdaPhiPlusError,      LambdaPhiPlusError      + 18 );
  Double_t MaxLambdaThetaPhiPlusError  = *max_element( LambdaThetaPhiPlusError, LambdaThetaPhiPlusError + 18 );
  Double_t MinLambdaThetaMinError      = *max_element( LambdaThetaMinError,     LambdaThetaMinError     + 18 );
  Double_t MinLambdaPhiMinError        = *max_element( LambdaPhiMinError,       LambdaPhiMinError       + 18 );
  Double_t MinLambdaThetaPhiMinError   = *max_element( LambdaThetaPhiMinError,  LambdaThetaPhiMinError  + 18 );
