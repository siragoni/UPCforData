...
void PolarisationSigExtrSystematicsHE(){
  ...
  for ( Int_t SigExBin = 0; SigExBin < 6; SigExBin++ ) {
    for ( Int_t FitRangeMode = 0; FitRangeMode < 3; FitRangeMode++ ) {
      FitResultFile[SigExBin][FitRangeMode] = new TFile(
            Form( "pngResults/%d-%2.2d-%2.2d/2DCS/Parameters_SigEx_%d_FitRange_%d_CS.root",
  ...
  graph1->Draw("AP");
  gPad->SaveAs("pngResults/CosThetaSysExtractCS_2D.png", "recreate");
  // canvas1->Update();
  ...
  // Draw the graph on the canvas.
  graph2->Draw("AP");
  gPad->SaveAs("pngResults/PhiSysExtractCS_2D.png", "recreate");
  // canvas1->Update();
  ...
  // Draw the graph on the canvas.
  graph3->Draw("AP");
  gPad->SaveAs("pngResults/TildeSysExtractCS_2D.png", "recreate");
  // canvas1->Update();
