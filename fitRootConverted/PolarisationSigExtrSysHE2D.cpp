 ...
 * -
 */
void PolarisationSigExtrSystematicsHE(){
  ...
  for ( Int_t SigExBin = 0; SigExBin < 6; SigExBin++ ) {
    for ( Int_t FitRangeMode = 0; FitRangeMode < 3; FitRangeMode++ ) {
      FitResultFile[SigExBin][FitRangeMode] = new TFile(
            Form( "pngResults/%d-%2.2d-%2.2d/2DHE/Parameters_SigEx_%d_FitRange_%d_HE.root",
  ...
  graph1->Draw("AP");
  gPad->SaveAs("pngResults/CosThetaSysExtractHE_2D.png", "recreate");
  // canvas1->Update();
  ...
  // Draw the graph on the canvas.
  graph2->Draw("AP");
  gPad->SaveAs("pngResults/PhiSysExtractHE_2D.png", "recreate");
  // canvas1->Update();
  ...
  // Draw the graph on the canvas.
  graph3->Draw("AP");
  gPad->SaveAs("pngResults/TildeSysExtractHE_2D.png", "recreate");
  // canvas1->Update();
