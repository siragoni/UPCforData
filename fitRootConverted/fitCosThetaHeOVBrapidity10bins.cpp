                       ...
                       int iRapidityBin             )
{
   ...
   */
  TH1F *fAngularDistribOfPositiveMuonRestFrameJPsiH = (TH1F*)listingsData->FindObject( Form("fCosThetaHelicityFrameJPsiTenRapidityBinsH_%d", iRapidityBin) );
  ...
  TH1F *fMCAngularDistribOfPositiveMuonRestFrameJPsiH = (TH1F*)listingsMC->FindObject( Form("fCosThetaHelicityFrameJPsiTenRapidityBinsH_%d", iRapidityBin) );
  ...
  TH1F *fMCthetaDistribOfPositiveMuonRestFrameJPsiGeneratedTruthH = (TH1F*)listingsMC->FindObject( Form("fMCCosThetaHelicityFrameJPsiTenRapidityBinsH_%d", iRapidityBin) );
  ...
  // TH1F *fMCrapidityBinGenerated = (TH1F*)listingsMC->FindObject( Form("fMCCosThetaHelicityFrameJPsiRapidityBinsH_%d", iRapidityBin) );
...
//  TF1* ParabolicFit = new TF1("ParabolicFit","[0]*(1-[1]+(3*[1]-1)*x*x)",-1., 1.);
  TF1* ParabolicFit = new TF1("ParabolicFit","[0]*(1+[1]+(1-3*[1])*x*x)",-1., 1.);
  ...
  fCorrectedShape->GetXaxis()->SetRangeUser(-0.7, /*fCorrectedShape->GetMaximum()*10.*/0.7);
  // gPad ->SetLogy();
  gStyle->SetOptFit(0);
  gStyle->SetOptStat(0);
  ...
  latex->DrawLatex(0.55,0.78,  Form(   "%.3f < y < %.3f",
                                       -4.0+1.5*(iRapidityBin    )/10,
                                       -4.0+1.5*(iRapidityBin + 1)/10
                                     )
                                  );
  ...
  gPad->SaveAs(Form("pngResults/CosThetaHe10Bins_%d.png", iRapidityBin), "RECREATE");

  new TCanvas;
  acceptance->Draw("ep");
  gPad->SaveAs(Form("pngResults/CosThetaHe10BinsACCxEFF_%d.png", iRapidityBin), "RECREATE");

  // new TCanvas;
  // fMCrapidityBinGenerated->Draw("ep");
  // gPad->SaveAs(Form("pngResults/CosThetaHeOvbGenerated_%d.png", iRapidityBin), "RECREATE");
