...
void fitHelicityCorrected2D(){
  ...
  // TFile* fileList = new TFile("AnalysisResultsMC24062019.root");
  TFile* fileList = new TFile("AnalysisResultsMC27062019.root");
  ...
  TFile* fileDataRaw = new TFile("pngResults/TH2variableBinningPerfect.root");
  ...
  TFile f("pngResults/TH2corrPerfect2.root", "recreate");
  acceptance->Write();
  RawH      ->Write();
  // AccErrors ->Write();
  f.Close();
