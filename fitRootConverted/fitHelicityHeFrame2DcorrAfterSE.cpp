...
/* - Fit function for the helicity case. It is basically a parabolic fit...
   -
 */
void fitHelicityShape(){
  // TFile* data = new TFile("pngResults/TH2corr.root");
  // TFile* data = new TFile("pngResults/TH2corrPerfect.root");
  TFile* data = new TFile("pngResults/TH2corrPerfect2.root");
  TH2F *fCorrectedShape = (TH2F*) data->Get("RawH");
  TF2* ParabolicFit = new TF2("ParabolicFit",helicity2D,-0.6, 0.3, -3.1, 3.1, 4);
  ...
  gPad->SetMargin(0.08,0.01,0.12,0.01);
  ...
  gPad->SaveAs("pngResults/fit2DcorrAfterSEnew.png",         "RECREATE");
