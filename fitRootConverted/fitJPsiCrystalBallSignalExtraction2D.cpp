...
void fitJPsiTemplate( const int selectionFlag, const int selectionFlag2 ){
  ...
  // } else {
    ...
    numberOfTotalJPsi     = (JPsiPeakFit    -> Integral(2.2,6))/0.08;
    numberOfTotalPsi2s    = (PsiPrimePeakFit-> Integral(2.2,6))/0.08;
    // if( selectionFlag < 16 || selectionFlag > 23 ) {
  ...
  gPad->SaveAs(Form("pngResults/Functional2DSignal_%d_%d.png", selectionFlag, selectionFlag2), "recreate");
...
void CreateCosThetaTh1(const char* AnalysisName){
  ...
  TFile f("pngResults/TH2variableBinningPerfect.root", "recreate");
