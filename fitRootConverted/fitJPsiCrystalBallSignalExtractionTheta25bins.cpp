...
void fitJPsiTemplate(const int selectionFlag){
  ...
  fInvariantMassDistributionH = (TH1F*)listings->FindObject( Form("fInvariantMassDistributionOnlyCosThetaForSignalExtractionHelicityFrameForAlreadyCorrectedFiftyH_%d", 2*selectionFlag) );
  TH1F* fInvariantMassDistributionTwoH = (TH1F*)listings->FindObject( Form("fInvariantMassDistributionOnlyCosThetaForSignalExtractionHelicityFrameForAlreadyCorrectedFiftyH_%d", 2*selectionFlag+1) );
  fInvariantMassDistributionH->Add(fInvariantMassDistributionTwoH);
  ...
  gPad->SaveAs(Form("pngResults/FunctionalSignalCosTheta25bins_%d.png", selectionFlag), "recreate");
...
void CreateCosThetaTh1(const char* AnalysisName){
  ...
  for (size_t iCosThetaBins = 4; iCosThetaBins < 21; iCosThetaBins++) {
