...
void fitJPsiTemplate(const char* AnalysisName, const int selectionFlag, const int selectionFlag2){
  ...
  // } else {
    ...
    JPsiRangeVar[0]       = (JPsiPeakFit    -> Integral(2.85,3.35))/0.02;
    JPsiRangeVar[1]       = (JPsiPeakFit    -> Integral(2.8,3.35))/0.02;
    JPsiRangeVar[2]       = (JPsiPeakFit    -> Integral(2.9,3.35))/0.02;
    JPsiRangeVar[3]       = (JPsiPeakFit    -> Integral(2.85,3.3))/0.02;
    JPsiRangeVar[4]       = (JPsiPeakFit    -> Integral(2.85,3.4))/0.02;
  ...
  numberOfTotalBkg    = (GammaGammaFit-> Integral(2.2,6))/0.02;
  ...
  gPad->SaveAs(Form("pngResults/InvMassSystematicsHalfHalfBin_%d_%d.png", selectionFlag, selectionFlag2), "RECREATE");
  ...
  TFile* fileSyst = new TFile("pngResults/SystematicsHalfHalfBin.root");
  ...
  // fileSyst->cd();
  // fileSyst->Close();
  TFile* fileSyst2 = new TFile("pngResults/SystematicsHalfHalfBin2.root", "recreate");
  fileSyst2->cd();
