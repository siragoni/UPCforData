...
void fitJPsiTemplate(const char* AnalysisName, const int selectionFlag){
  ...
  latex->DrawLatex(0.55,0.78,Form("Signal_%d", selectionFlag));
  ...
  gPad ->SaveAs(Form("pngResults/fitSignalExtractionCosTheta_%d.png", selectionFlag),     "RECREATE");
...
void CreateTH2(const char* AnalysisName){
  ...
  for (size_t iCosThetaBins = 8; iCosThetaBins < 32; iCosThetaBins++) {
    // for (size_t iPhiBins = 0; iPhiBins < 10; iPhiBins++) {
      ...
      BkgValue         = 0;
      BkgValueError    = 0;
      fitJPsiTemplate(AnalysisName, iCosThetaBins);
  ...
  TFile f("pngResults/TH1signalCosThetaEX.root", "recreate");
