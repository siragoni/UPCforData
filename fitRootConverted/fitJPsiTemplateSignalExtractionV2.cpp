...
void fitJPsiTemplate(const char* AnalysisName, const int selectionFlag, const int selectionFlag2){
  ...
  // TDirectory* dir = fileList->GetDirectory("MyTask");
  // TFile* fileMC[8];
  // fileMC[0] = new TFile("MCtrainResults/18032019/kCohJpsiToMu/AnalysisResults.root");
  // fileMC[1] = new TFile("MCtrainResults/18032019/kCohPsi2sToMu/AnalysisResults.root");
  // fileMC[2] = new TFile("MCtrainResults/18032019/kCohPsi2sToMuPi/AnalysisResults.root");
  // fileMC[3] = new TFile("MCtrainResults/18032019/kIncohJpsiToMu/AnalysisResults.root");
  // fileMC[4] = new TFile("MCtrainResults/18032019/kIncohPsi2sToMu/AnalysisResults.root");
  // fileMC[5] = new TFile("MCtrainResults/18032019/kIncohPsi2sToMuPi/AnalysisResults.root");
  // fileMC[6] = new TFile("MCtrainResults/18032019/kTwoGammaToMuHigh/AnalysisResults.root");
  // fileMC[7] = new TFile("MCtrainResults/18032019/kTwoGammaToMuMedium/AnalysisResults.root");
  // TDirectory* dirMC[8];
  ...
  fInvariantMassDistributionH = (TH1F*)listings->FindObject( Form("fInvariantMassDistributionForSignalExtractionHelicityFrameH_%d_%d", selectionFlag, selectionFlag2) );
  ...
  latex->DrawLatex(0.55,0.78,Form("Signal_%d_%d", selectionFlag, selectionFlag2));
  ...
  gPad ->SaveAs(Form("pngResults/fitSignalExtraction_%d_%d.png", selectionFlag, selectionFlag2),     "RECREATE");
  ...
  // l->Draw();
  //
  // if      ( selectionFlag == 0 ) gPad->SaveAs("pngResults/PtIntJPsiLeg.png",      "RECREATE");
  // else if ( selectionFlag == 1 ) gPad->SaveAs("pngResults/CoherentJPsiLeg.png",   "RECREATE");
  // else if ( selectionFlag == 2 ) gPad->SaveAs("pngResults/IncoherentJPsiLeg.png", "RECREATE");
...
void CreateTH2(const char* AnalysisName){
  ...
  for (size_t iCosThetaBins = 2; iCosThetaBins < 8; iCosThetaBins++) {
    for (size_t iPhiBins = 0; iPhiBins < 10; iPhiBins++) {
      JPsiPeakValue    = 0;
      JPsiPeakValueErr = 0;
      BkgValue         = 0;
      BkgValueError    = 0;
      fitJPsiTemplate(AnalysisName, iCosThetaBins, iPhiBins);
      // helicity2DafterSignalExtraction->Fill( CosThetaBinCenters[iCosThetaBins],
      //                                        PhiBinCenters[iPhiBins],
      //                                        JPsiPeakValue
      //                                        );
      // helicityOfGammaGamma->Fill( CosThetaBinCenters[iCosThetaBins],
      //                             PhiBinCenters[iPhiBins],
      //                             BkgValue
      //                             );

      helicity2DafterSignalExtraction->Fill( -0.9   + (Double_t)iCosThetaBins * 2.0   / 10.0,
                                             // -3.6 + (Double_t)iPhiBins      * 8.0 / 10.0,
                                             -2.826 + (Double_t)iPhiBins      * 0.628,
                                             // -3.14 + 0.314 * ( 2.0 * (Double_t)iPhiBins + 1.0) ,
                                             JPsiPeakValue
                                             );
      helicityOfGammaGamma->Fill( -0.9   + (Double_t)iCosThetaBins * 2.0  / 10.0,
                                  // -3.6 + (Double_t)iPhiBins      * 8.0 / 10.0,
                                  -2.826 + (Double_t)iPhiBins      * 0.628,
                                  // -3.14 + 0.314 * ( 2.0 * (Double_t)iPhiBins + 1.0) ,
                                  BkgValue
      ...
      helicity2DafterSignalExtractionErrors->Fill( -0.9   + (Double_t)iCosThetaBins * 2.0   / 10.0,
                                                   // -3.6 + (Double_t)iPhiBins      * 8.0 / 10.0,
                                                   -2.826 + (Double_t)iPhiBins      * 0.628,
                                                   // -3.14 + 0.314 * ( 2.0 * (Double_t)iPhiBins + 1.0) ,
                                                   JPsiPeakValue
                                                   );
      helicityOfGammaGammaErrors->Fill( -0.9   + (Double_t)iCosThetaBins * 2.0  / 10.0,
                                        // -3.6 + (Double_t)iPhiBins      * 8.0 / 10.0,
                                        -2.826 + (Double_t)iPhiBins      * 0.628,
                                        // -3.14 + 0.314 * ( 2.0 * (Double_t)iPhiBins + 1.0) ,
                                        BkgValue
  ...
  TFile f("pngResults/TH2signalEX.root", "new");
