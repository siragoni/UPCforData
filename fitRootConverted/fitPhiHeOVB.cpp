...
void fitHelicityShape(const char* AnalysisName, const char* MonteCarloName){
   ...
   */
  TH1F *fAngularDistribOfPositiveMuonRestFrameJPsiH = (TH1F*)listingsData->FindObject("fPhiHelicityFrameJPsiH");
  ...
  TH1F *fMCAngularDistribOfPositiveMuonRestFrameJPsiH = (TH1F*)listingsMC->FindObject("fPhiHelicityFrameJPsiH");
  ...
  */
  ...
  fCorrectedShape->GetYaxis()->SetRangeUser(0, 100.);
  ...
  latex->DrawLatex(0.55,0.72,Form("#lambda_{#phi} = %.4f #pm %.4f",
  ...
  gPad->SaveAs("pngResults/PhiHeOvb.png", "RECREATE");

  new TCanvas;
  acceptance->Draw("ep");
  gPad->SaveAs("pngResults/PhiHeOvbACCxEFF.png", "RECREATE");
