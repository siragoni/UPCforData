...
//296977 = bad ZDC?
//296376 = bad AD (cosmic-like config)


void lumiEvgenyStyle(){
  TH1F* hTrigRecorded = new TH1F("hTrigRecorded","",nRuns,0,nRuns);
  TH1F* hTrigAnalysed = new TH1F("hTrigAnalysed","",nRuns,0,nRuns);
  TH1F* hLumiRecorded = new TH1F("hLumiRecorded","",nRuns,0,nRuns);
  TH1F* hLumiAnalysed = new TH1F("hLumiAnalysed","",nRuns,0,nRuns);
  Int_t run;
  ...
  Double_t sigmaINEL = 7.66; // Inelastic Pb-Pb cross section [b]
  for (Int_t i=0;i<nRuns;i++){
    ...
    printf("%i\n",run);
    TString className = "CMUP11-B-NOPF-MUFAST";
    ...
    Double_t sigma = sigmaINEL*eff;
    if (295671 <=run && run<= 295715) l0bRef/=0.135; // bug in trigger config
    if (295717 <=run && run<= 295720) l0bRef/=0.135; // bug in trigger config

    Double_t lumiSeen = l0bRef/sigma*mu/(1-exp(-mu));
    Double_t lt = l2aLT/l0bLT;
    Double_t lumiRecorded = lumiSeen*lt;
    // TFile* f = new TFile("AnalysisResultsLHC18qr15o29052019noSPD.root");
    // TList* list = (TList*) f->Get("MyTask/MyOutputContainer"); // provide your list here
    // // TH2D* fTriggersVsRun = (TH2D*) list->FindObject("fTriggersVsRun");
    // TH2F* fTriggersVsRun = (TH2F*) list->FindObject("fTriggersVsRunH");
    Int_t class_bin = 0;
    ...
    Int_t run_bin   = fTriggersVsRun->GetYaxis()->FindFixBin(run+0.5);
    Int_t nTrigAnalysed = fTriggersVsRun->GetBinContent(class_bin,run_bin);
  ...
  l->AddEntry(hLumiRecorded,Form("Recorded = %.3f /ub",hLumiRecorded->Integral()));
  l->AddEntry(hLumiAnalysed,Form("Analysed = %.3f /ub",hLumiAnalysed->Integral()));
  l->Draw();

  printf("%f\n",hLumiAnalysed->Integral());
  TFile* outputFile = new TFile("lumi.root","recreate");
  hTrigRecorded->Write();
  hTrigAnalysed->Write();
  hLumiRecorded->Write();
  hLumiAnalysed->Write();
